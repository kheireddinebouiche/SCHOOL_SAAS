...
def delete_pv(request):
    if request.method == "POST":
        ...
        try:
            ...
            # Check if PV exists for this exam plan
            if hasattr(exam_plan, 'pv'):
                pv_examen = exam_plan.pv

                # Check if PV is validated - if so, it cannot be deleted
                ...
                return JsonResponse({"status": "error", "message": "Aucun PV trouvé pour cette planification"})
