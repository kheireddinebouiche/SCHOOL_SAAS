...
files_to_update = [
    ...
    r"templates/tenant_folder/exams/deliberation.html",
...
for f in files_to_update:
    if os.path.exists(f):
        ...
        # Replace existing style blocks if they exist (heuristically)
        # Just insert the glass-card styles right after {% block content %}
        ...
        # specifically replace .stats-card with .stat-card
        content = content.replace('stats-card', 'stat-card')
        
        # Also clean up classes that clash with glass-card
        ...
        content = content.replace('card border-0 shadow-sm rounded-3', 'glass-card')
        content = content.replace('class="card session-card h-100 border-0 shadow-sm"', 'class="glass-card session-card h-100"')
        
        # Replace the header background block
        content = re.sub(
            r'bg-gradient-to-r from-primary to-info rounded-3 shadow p-4 text-white',
            r'glass-card p-4',
            content
        )
        content = re.sub(
            r'bg-white bg-opacity-20 rounded-circle p-3 me-3',
            r'icon-box bg-primary bg-opacity-10 text-primary me-3',
            content
        )
        content = re.sub(
            r'text-white text-opacity-75',
            r'text-muted',
            content
        )
        content = re.sub(
