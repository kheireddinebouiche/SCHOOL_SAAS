...
for tenant in TenantModel.objects.exclude(schema_name='public'):
    with schema_context(tenant.schema_name):
        ...
        # Depenses
        depenses = Depenses.objects.filter(mode_paiement='che')
        for d in depenses:
            statut = 'emis'
        ...
        for r in rembs:
            statut = 'emis'
            if r.is_appliced or r.lettrages.filter(is_rapproche=True).exists():
                statut = 'decaisse'
        ...
        if count_d + count_r > 0:
            print(f"Tenant {tenant.schema_name}: processed {count_d} depenses, {count_r} remboursements")
        total_processed += count_d + count_r

print(f"Backfill complete. Total checks processed: {total_processed}")
