...
# Basic HTML structure for a Dolibarr-like invoice (facture)
dolibarr_facture_html = """
    ...
    <table style="width: 100%; border-collapse: collapse; margin-bottom: 30px;">
        <tr>
            ...
            <td style="width: 50%; vertical-align: top; text-align: right;">
                <h2 style="color: #2c3e50; font-size: 20px; margin: 0 0 10px 0;">FACTURE</h2>
                <p style="margin: 0; line-height: 1.5; font-weight: bold;">
                    Réf: {{ facture_numero }}<br>
                ...
                <div style="margin-top: 20px; text-align: left; background: #f8f9fa; padding: 15px; border: 1px solid #dee2e6; border-radius: 4px; display: inline-block; min-width: 250px;">
                    <h3 style="margin: 0 0 10px 0; font-size: 14px; color: #2c3e50;">Facturé à:</h3>
                    {% if client_logo %}
        ...
        <tr>
            <td style="width: 60%; vertical-align: top;">
                ...
                <div style="margin-top: 20px;">
                    <h4 style="margin: 0 0 5px 0; font-size: 13px;">Mode de paiement :</h4>
                    <p style="margin: 0; font-size: 11px; color: #555; white-space: pre-wrap;">{{ mode_paiement }}</p>
                </div>
                {% endif %}
                ...
                {% if banque_iban %}
                <div style="margin-top: 20px;">
                    <h4 style="margin: 0 0 5px 0; font-size: 13px;">Coordonnées bancaires :</h4>
                    <p style="margin: 0; font-size: 11px; color: #555;">
                        <strong>Banque :</strong> {{ banque_nom }}<br>
                        <strong>IBAN :</strong> {{ banque_iban }}
                    </p>
                </div>
                {% endif %}
            </td>
            <td style="width: 40%;">
                <table style="width: 100%; border-collapse: collapse;">
                    ...
                    {% if timbre > 0 %}
                    <tr>
                        <td style="padding: 5px; text-align: right;">Droit de Timbre :</td>
                        <td style="padding: 5px; text-align: right;">{{ timbre|floatformat:2 }}</td>
                    </tr>
                    {% endif %}
                    <tr>
...
for tenant in Institut.objects.exclude(schema_name='public'):
    with tenant_context(tenant):
        doc, created = DocumentTemplate.objects.update_or_create(
            slug='dolibare_facture',
            defaults={
                'title': 'Facture Dolibarr',
                'template_type': 'invoice',
                'content': dolibarr_facture_html,
                'description': 'Modèle de facture style Dolibarr (Azur/Crabe)',
                'is_active': True,
                'custom_css': '@page { margin: 0.5cm; }'
            }
        )
        if created:
            print(f"[{tenant.schema_name}] Template 'dolibare_facture' created.")
        else:
            print(f"[{tenant.schema_name}] Template 'dolibare_facture' updated.")
